...
const SavePost = () => {
    ...
    async function fetchUserPosts() {
      try {
        const res = await axios.post(
          "http://localhost:5000/post/fetch-user-saved-posts",
          { id: user._id },
          config
        );

        if (res.status === 200) {
          setMySavedPosts(res.data.posts.savedPosts);
          setIsLoading(false);
        }
      } catch (error) {
        console.log(error);
  ...
  return (
    <div className="container pt-5 pb-3">
      {mySavedPosts?.length === 0 && !isLoading ? (
        ...
        <>
          <h1 className="text-center mt-3 py-2 mb-5">
            <span className="headings">My Saved Posts </span>
          </h1>
